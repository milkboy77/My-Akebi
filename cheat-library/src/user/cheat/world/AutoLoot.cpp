...
namespace cheat::feature 
{
    ...
    void AutoLoot::DrawMain()
    {
		if (ImGui::BeginTable("AutoLootDrawTable", 2, ImGuiTableFlags_NoBordersInBody))
		{
			...
			ImGui::BeginGroupPanel(u8"自动宝箱");
			{
				ConfigWidget(u8"开关", f_AutoTreasure, u8"自动打开箱子和其他宝藏。\n" \
					...
					u8"如果与自定义范围一起使用，请确保先启用该选项.");
				ImGui::SameLine();
				ImGui::TextColored(ImColor(255, 165, 0, 255), u8"读这张消息！");
				ImGui::Indent();
				ConfigWidget(u8"宝箱", f_Chest, u8"普通、珍贵、华丽等。");
				ConfigWidget(u8"地脉", f_Leyline, u8"莫拉/经验书/大世界boos,等等..");
				ConfigWidget(u8"调查点", f_Investigate, u8"标记为调查/搜索等");
				ConfigWidget(u8"任务交互", f_QuestInteract, u8"有效的任务是与点进行交互。");
    	...
    	ImGui::BeginGroupPanel(u8"拾取类型");
	    {
			...
			ConfigWidget(u8"动物", f_PickupFilter_Animals, u8"鱼、蜥蜴、青蛙和飞行动物。"); ImGui::SameLine();
			ConfigWidget(u8"掉落物", f_PickupFilter_DropItems, u8"材料、矿物或人工制品。"); ImGui::SameLine();
