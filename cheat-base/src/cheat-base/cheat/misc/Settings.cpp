...
namespace cheat::feature
{
	Settings::Settings() : Feature(),
		...
		NF(f_NotificationsShow, u8"显示通知", "General::Notify", true),
		NF(f_NotificationsDelay, u8"延迟通知", "General::Notify", 500),

		NF(f_FileLogging, u8"文件记录", "General::Logging", false),
		NF(f_ConsoleLogging, u8"控制台记录", "General::Logging", true),

		NF(f_FastExitEnable, u8"快速退出", "General::FastExit", false),
		NF(f_HotkeyExit, u8"快捷键", "General::FastExit", Hotkey(VK_F12)),
	...
	void Settings::DrawMain()
	{
		...
		ImGui::BeginGroupPanel(u8"自定义窗口");
		{
			...
			ImGui::InputText(u8"颜色名称", &nameBuffer_);
